  ...
  <div class="max-w-4xl mx-auto mt-8 p-4 bg-gray-50 rounded-lg shadow-md">
    ...
    <div class="relative w-full flex justify-center items-center" :style="{ height: `${height}px`, width: `${width}px` }">
      ...
      <div 
        ...
        :style="[getUserPositionStyle(relationship.user.id, index, relationships.length), { width: '60px', height: '60px' }]">
        <span class="text-xs text-center font-semibold leading-tight">{{ relationship.user.name }} {{ relationship.user.last_name }}</span>
      </div>
      ...
      <div 
        ...
        :style="[getUserPositionStyle(relationship.peer.id, index, relationships.length), { width: '60px', height: '60px', zIndex: 1 }]">
        <span class="text-xs text-center font-semibold leading-tight">{{ relationship.peer.name }} {{ relationship.peer.last_name }}</span>
      </div>
      ...
      <svg class="absolute top-0 left-0 w-full h-full" style="z-index: 0;">
        <line 
          ...
          :stroke="relationship.relationship_type === 'positive' ? '#34D399' : '#F87171'"
          stroke-width="2"
        />
      </svg>
    </div>
