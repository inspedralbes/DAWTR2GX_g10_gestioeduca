...
export const useFormsStore = defineStore('forms', {
  ...
  actions: {
    ...
    async loadFormById(formId) {
      ...
      try {
        const response = await fetch(`http://grupify.daw.inspedralbes.cat/backend/public/api/forms/${formId}`, {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('auth_token')}`,
            'Accept': 'application/json',
          },
    ...
    async getFormQuestionsAndAnswers(formId) {
      ...
      try {
        const response = await fetch(`http://grupify.daw.inspedralbes.cat/backend/public/api/forms/${formId}/questions`, {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('auth_token')}`,
            'Accept': 'application/json',
          },
