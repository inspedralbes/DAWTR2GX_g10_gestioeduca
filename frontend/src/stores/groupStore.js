  ...
  actions: {
    async fetchGroups() {
      try {
        ...
        const response = await fetch('http://pruebag10.daw.inspedralbes.cat/backend/public/api/groups', {
          method: 'GET',
          headers: {
        ...
        this.groups.forEach(async (group) => {
          const membersResponse = await fetch(`http://pruebag10.daw.inspedralbes.cat/backend/public/api/groups/${group.id}/members`, {
            method: 'GET',
            headers: {
              'Authorization': `Bearer ${token}`,
              'Accept': 'application/json',
